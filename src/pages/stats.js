...
const Stats = () => {
  ...
  const getDataPersonal = async (e) => {
    ...
    const personalResponse = await response.data.filter(function (x) {
  ...
  const getDataMatchPersonal = async (e) => {
    ...
    const personalResponse = await responseSource.data.filter(function (x) {
      return x.player1Address === value.state.walletFromDb;
    });
    const personalResponse2 = await responseSource.data.filter(function (x) {
